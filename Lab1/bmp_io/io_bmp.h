...
extern int bmp_out__put_line(bmp_out *state, io_byte *line);
  /* Writes the next line of image data to the file opened using the most
	 ...
	 /*****************************************************************************/
struct io_comp {
	int width, height, stride; // width and stride are different so that additional space is set outside the image bounds
	int *buf, *handle;		// Buf points to the top left image corner, handle points to the start of the allocated memory.
};
