...
{
  io_int32 test = 1;
  io_byte *first_byte = (io_byte *) &test; // MSB is that highest memory address. Therefore first_byte currently points to LSB == 1
  if (*first_byte)
...
int bmp_in__open(bmp_in *state, const char *fname)
{
  ...
  io_byte magic[14];
  bmp_header header;
  fread(magic,1,14,state->in); // Read in the first 14 Bytes preceeding the bmp header
  if ((magic[0] != 'B') || (magic[1] != 'M')) // Check if it's a legit bmp file
    return(IO_ERR_FILE_HEADER);
  if (fread(&header,1,40,state->in) != 40) // Now read the 40 byte header
    return(IO_ERR_FILE_TRUNC);

  from_little_endian((io_int32 *) &header,10); // Take each 10 of the 4-byte data blocks and turn it to little endian
  state->cols = header.width;
  state->rows = header.height;
  int bit_count = (header.planes_bits>>16); // Remember plane_bits == 16_16
  if (bit_count == 24)
    state->num_components = 3; //BGR
  else if (bit_count == 8) // 8 bit grey
  ...
  else if (header.num_colours_used == 0) // Use default
    palette_entries_used = (1<<bit_count);
  int header_size = 54 + 4*palette_entries_used; //14+40+colour table entries, each 4 bytes large

  int offset = magic[13];  // Little endian
  offset <<= 8; offset += magic[12];
  offset <<= 8; offset += magic[11];
  offset <<= 8; offset += magic[10];  // offset = magic [13][12][11][10]
...
int bmp_out__open(bmp_out *state, const char *fname,
                  int width, int height, int num_components) // Arguments required to set up bmp header.
{
