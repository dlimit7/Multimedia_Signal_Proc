  ...
  try {
      ...
      bmp_in__close(&in);
		...
		for (int n_sigma = 0; n_sigma < N; n_sigma++)
		{
			...
			for (n = 0; n < num_comps; n++)
				intermediate_comps[n].init(height, width, 1); // Don't need a border for output

			my_image_comp *output_comps = new my_image_comp[num_comps];
			for (n = 0; n < num_comps; n++)
				output_comps[n].init(height, width, 1); // Don't need a border for output
			...
			int A[9] = { 0, 1, 0, 1, 1, 1, 0, 1, 0 };
			for (n = 0; n < num_comps; n++)
				input_comps[n].perform_boundary_extension();
			for (n = 0; n < num_comps; n++) {
				laplacian.apply_filter(input_comps + n, intermediate_comps + n);
				laplacian.apply_dilation(intermediate_comps + n, output_comps + n, A, 3);
			}
			...
			{ // "r" holds the true row index we are writing, since the image is
				...
				for (n = 0; n < num_comps; n++)
				{
					...
					float *src = intermediate_comps[n].buf + r * intermediate_comps[n].stride;
			...
			sprintf(file_out, "%s_%d_dilated.bmp", argv[2], n_sigma);
			if ((err_code = bmp_out__open(&out, file_out, width, height, num_comps)) != 0)
				throw err_code;
			for (r = height - 1; r >= 0; r--)
			{ // "r" holds the true row index we are writing, since the image is
